...
-(void)requestHouseList
{
    ...
    //设置参数
    NSDictionary *pramater = @{@"HEAD_INFO":value};
    [manager GET: baseURL                                                                                                                                                                                                           parameters:pramater progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"==%@==",responseObject);
        //json解析
        NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingAllowFragments error:nil];
        NSLog(@"==%@==",dict);
        NSArray *temp = dict[@"RESPONSE_BODY"][@"list"];
        /*
         * 缓存
         *将房租列表存在本地，用于下次打开页面，直接从本地获取
         *
         */
...
{
    //1.读取本地转存http://www.fungpu.com/houseapp/Photos/201501/e646b1e498ed4d8781076403eea5bb65.jpg
      //1.1 截取字符串获取图片名称
    ...
    if (arr.count > 0)
    {
        imgName = arr[arr.count - 2];
      //1.4获取到本地酒店
        UIImage *image = [[FileHandle shareHandel] loadImageFormLocal:imgName];
        if (image) {
            //更新UI
            if (cell)
            {
  ...
  //  manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
    ...
    [manager GET:Url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        //2.把图片存在本地
        [[FileHandle shareHandel] saveLocalImage:responseObject saveName:imgName];
        NSLog(@"--%@--",[FileHandle shareHandel]);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSLog(@"==error==%@",error);
    }];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //1.拼接图片的URL
    NSString *imgUrl = [ImageURL stringByAppendingPathComponent:cell.dic[@"iconurl"]];
    //2.加载图片
    [self loadImageFromNetWork:imgUrl indexPath:cell];
